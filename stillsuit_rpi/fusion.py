...
from math import sqrt, atan2, asin, degrees, radians,acos   
...
class Fusion(object):
    ...
    def update_nomag(self, accel, gyro, timediff, ts=None):    # 3-tuples (x, y, z) for accel, gyro
        ...
        # # if -asin(2.0 * (self.q[1] * self.q[3] - self.q[0] * self.q[2]))>0 and -acos(2.0 * (self.q[1] * self.q[3] - self.q[0] * self.q[2]))<0:
        # #     self.pitch=180-self.pitch
        # # elif -asin(2.0 * (self.q[1] * self.q[3] - self.q[0] * self.q[2]))<0 and -acos(2.0 * (self.q[1] * self.q[3] - self.q[0] * self.q[2]))>0:
        # #     self.pitch=-180+self.pitch
        # # else:
        # #     self.pitch
        ...
        #changed code:
        self.pitch = degrees(atan2(2.0 * (self.q[1] * self.q[3] - self.q[0] * self.q[2]),self.q[0] * self.q[0] - self.q[1] * self.q[1] - self.q[2] * self.q[2] + self.q[3] * self.q[3]))
        self.roll = degrees(-asin(2.0 * (self.q[0] * self.q[1] + self.q[2] * self.q[3])))

    def update(self, accel, gyro, mag,timediff, ts=None):     # 3-tuples (x, y, z) for accel, gyro and mag data
